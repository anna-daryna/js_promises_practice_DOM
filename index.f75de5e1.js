...
const success = (message)=>{
    document.body.insertAdjacentHTML("afterbegin", `<div class="success" data-qa="notification">${message}</div>`);
};
const error = (message)=>{
    document.body.insertAdjacentHTML("afterbegin", `<div class="error" data-qa="notification">${message}</div>`);
};
firstPromise.then(()=>success("First promise was resolved")).catch(()=>error("First promise was rejected"));
